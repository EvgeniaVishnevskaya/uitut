...
const state = usePages('expandable').find(e=> e.children?.length).children
...
export default function Menu() {
    ...
    const [opened, setOpened] = useState(state)  
      const open = (i) => {
        const arr = opened
        if (arr[i].isOpen) arr[i].isOpen = false
        else {
          arr.forEach(el => {
            el.isOpen = false
          })
      ...
      return opened?.map((el, i) => {
        ...
        return (
          ...
          <div className={`flex gap-[8px] py-4 px-[24px] items-center cursor-pointer justify-between`}  onClick={()=> open(i)}>        
          { el.route ? <Link to={el.route}>
          ...
          <div className="pb-1">{el.isOpen ? <UpOutlined /> : <DownOutlined />}</div>
          </div>
          <div className="ml-5">
          { el.isOpen ?  el.children.map(child => {
            return (
            <Link to={child.route} key={child.label}>
              <div className={`border-l ${currPath === child.route? 'border-l-accent-blue text-accent-blue' :'border-l-main-disable text-text-header'} transition duration-300 flex gap-[8px] py-2 px-[24px] items-center`} > 
